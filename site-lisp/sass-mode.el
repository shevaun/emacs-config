...
; internal
(defun sass-list-2-regexp(altlist)
  "Takes a list and returns the regexp \\(elem1\\|elem2\\|...\\)"
  (let ((regexp "\\("))
    (mapcar (lambda(elem)
	      (setq regexp (concat regexp elem ":\\|:" elem "\\|")))
	    altlist)
...
(defconst sass-font-lock-keywords-1
  (list
   ;; urls
   '("url([^)]*)"  0 font-lock-string-face)
   ;; named constant
   '("![a-z0-9_]+"                    0 font-lock-variable-name-face)
   ;; color constant
   '("#[a-f0-9]+\\([ \t;]\\|$\\)"                    0 font-lock-reference-face)
   (cons "rgb(\\s-*[0-9]+\\(\\.[0-9]+\\s-*%\\s-*\\)?\\s-*,\\s-*[0-9]+\\(\\.[0-9]+\\s-*%\\s-*\\)?\\s-*,\\s-*[0-9]+\\(\\.[0-9]+\\s-*%\\s-*\\)?\\s-*)"
         font-lock-reference-face)
   ;; id
   '("^[^:]*\\(#[a-z0-9_]+\\)"   1 font-lock-function-name-face)
   ;; class
   '("\\([^0-9]\\|^\\)\\(\\.[a-z0-9_]+\\)"   2 font-lock-type-face)
   ;; attribute 
   (cons (sass-list-2-regexp cssm-properties) font-lock-keyword-face)
...
(defcustom sass-comment-column 32
  "*Indentation column of comments."
  :type 'integer :group 'sass)
(put 'sass-comment-column 'safe-local-variable 'integerp)
...
(if sass-mode-map
  ...
  (define-key sass-mode-map (read-kbd-macro "M-TAB") 'cssm-complete-property)
...
\\{sass-mode-map}"
  ...
  (set (make-variable-buffer-local 'comment-column) sass-comment-column)
  ...
  (save-excursion
    ...
    (let ((adjust (if (looking-at ".*}[[:space:]]*$")
      ...
      (if (bobp) 0
        ...
        (+ (current-indentation) 
           (if (looking-at ".*{[[:space:]]*$")
               (if (= 0 adjust) sass-indent-offset 0)
             adjust))))))
...
0, it will cycle back to the maximum sensible indentation."
  (interactive "*")
  (sass-indent-to (sass-compute-indentation)))

(defun sass-indent-to (indent)
  "Indent the current line."

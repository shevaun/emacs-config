...
(defcustom coffee-backspace-function 'backward-delete-char-untabify
  "*Function called by `coffee-electric-backspace' when deleting backwards."
  :type 'function
  :group 'coffee)
...
(defvar coffee-ident-regexp "\\<[a-zA-Z_0-9]*_[a-zA-Z_0-9]*\\>")
...
(defun coffee-compute-indentation ()
  ...
  (save-excursion
    (beginning-of-line)
    (let ((adjust (if (looking-at ".*}[[:space:]]*$")
                      (- coffee-indent-offset) 0)))
      (if (bobp) 0
        (forward-line -1)
        (while (and (looking-at coffee-blank-line-re)
                    (> (point) (point-min)))
          (forward-line -1))
        (+ (current-indentation) 
           (if (coffee-line-wants-indent)
               (if (= 0 adjust) coffee-indent-offset 0)
             adjust))))))
...
0, it will cycle back to the maximum sensible indentation."
  (interactive "*")
  (coffee-indent-to (coffee-compute-indentation)))

(defun coffee-indent-to (indent)
  "Indent the current line."
  (if (< (current-column) (current-indentation))
      (back-to-indentation)
    (message "%s,%s,%s,%d" last-command last-command (eq last-command 'indent-for-tab-command) indent)
    (when (eq last-command 'indent-for-tab-command) 
        (back-to-indentation)
        (setq indent (+ indent
                        (if (= (current-indentation) indent)
                            coffee-indent-offset
                          (if (> (current-indentation) indent)
                              (- coffee-indent-offset)
                            0)))))
...
(defun coffee-electric-backspace (arg)
  "Delete characters or back-dent the current line.
If invoked following only whitespace on a line, will back-dent to the
immediately previous multiple of `coffee-indent-offset' spaces."
  (interactive "*p")
  (if (or (/= (current-indentation) (current-column)) (bolp))
      (funcall coffee-backspace-function arg)
    (let ((ci (current-column)))
      (beginning-of-line)
      (delete-horizontal-space)
      (indent-to (* (/ (- ci (* arg coffee-indent-offset))
                       coffee-indent-offset)
                    coffee-indent-offset)))))
...
(defun coffee-unindent-block (arg &optional start end)
  "Unindents the block of nested statements or (if selected) the region"
  (interactive "*p\nr")
  (coffee-indent-block (- arg) start end))

(defun coffee-indent-block (arg &optional start end)
  "Indents the block of nested statements or (if selected) the region"
  (interactive "*p\nr")
  (let (li
        (ci (current-indentation))
        (offset (* arg coffee-indent-offset)))
    (save-excursion
      (if (region-active-p)
          (progn
            (goto-char start)
            (beginning-of-line)
            (while (< (point) end)
              (setq li (+ (current-indentation) offset))
              (delete-horizontal-space)
              (indent-to li)
              (forward-line)
              )
        ...
        (beginning-of-line)
        (while (and (not (eobp)) (or (>= (current-indentation) ci)
                                     (looking-at coffee-blank-line-re)
                                     ))
          (when (>= (current-indentation) ci)
            (setq li (+ (current-indentation) offset))
            (delete-horizontal-space)
            (indent-to li))
          (forward-line))))))
...
(defvar coffee-indenters-bol-regexp (regexp-opt coffee-indenters-bol 'words))
...
;;;###autoload
(defvar coffee:mode-syntax-table nil
  "Syntax table in use in coffee:mode buffers.")

(if coffee:mode-syntax-table
    ()
  (setq coffee:mode-syntax-table (make-syntax-table))
  (modify-syntax-entry ?\' "\"" coffee:mode-syntax-table)
  (modify-syntax-entry ?\" "\"" coffee:mode-syntax-table)
  (modify-syntax-entry ?\` "\"" coffee:mode-syntax-table)
  (modify-syntax-entry ?# "<" coffee:mode-syntax-table)
  (modify-syntax-entry ?\n ">" coffee:mode-syntax-table)
  (modify-syntax-entry ?\\ "\\" coffee:mode-syntax-table)
  (modify-syntax-entry ?$ "." coffee:mode-syntax-table)
  (modify-syntax-entry ?? "_" coffee:mode-syntax-table)
  (modify-syntax-entry ?_ "_" coffee:mode-syntax-table)
  (modify-syntax-entry ?< "." coffee:mode-syntax-table)
  (modify-syntax-entry ?> "." coffee:mode-syntax-table)
  (modify-syntax-entry ?& "." coffee:mode-syntax-table)
  (modify-syntax-entry ?| "." coffee:mode-syntax-table)
...
(define-derived-mode coffee-mode fundamental-mode
  ...
  (set-syntax-table coffee:mode-syntax-table)
  ...
  (define-key coffee-mode-map [backspace] 'coffee-electric-backspace)
  (define-key coffee-mode-map (kbd "C-M-'") 'coffee-indent-block)
  (define-key coffee-mode-map (kbd "C-M-;") 'coffee-unindent-block)
  ...
  (highlight-indentation coffee-indent-offset)

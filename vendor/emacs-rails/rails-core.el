...
(defun rails-core:file (file-name)
  ...
  (cond
   ((listp file-name)
    (while (let ((fn (rails-core:file (car file-name))))
             (if (file-exists-p fn)
                 (and (setq file-name fn)
                      nil)
               t))
...
(defconst rails-core:gem-trace-re  "^[[:blank:]]*\\[?\\(\\([^/[:blank:]][^:(\n]+ ([^)]+) [.a-zA-Z0-9_/-]+\\):\\([0-9]+\\)\\):")

(defun rails-core:goto-backtrace ()
  (interactive)
    (save-match-data
      (save-excursion
        (beginning-of-line)
        (let* ((location
                (cond
                 ((looking-at rails-core:gem-trace-re)
                  (let ((fd (rails-test:gem-find)))

                    (when (cdr fd) (cons (concat (cdr fd) "/" (car fd)) (match-string-no-properties 3)))))

                 (t
                  (search-forward-regexp "[[]?\\(\\(/\\)?[.a-zA-Z0-9_/-]+\\):\\([0-9]+\\)")
                  (if (string= (match-string-no-properties 2) "/")
                      (cons (match-string-no-properties 1) (match-string-no-properties 3))
                    (cons (concat (rails-project:root) (match-string-no-properties 1)) (match-string-no-properties 3))))))
               (buffer (and (car location) (file-exists-p (car location)) (find-file-noselect (car location)))))
          (when buffer
            (switch-to-buffer-other-window buffer)
            (goto-char (point-min)) (forward-line (1- (string-to-number (cdr location)))))))))
...
(defun rails-core:model-by-migration-filename (migration-filename)
  (when migration-filename
    (let ((model-name (singularize-string
                       (nth-match "[0-9]+_create_\\([a-zA-Z_0-9]+\\)\.rb" migration-filename 1))))
      (when (and model-name
...
(defun rails-core:coffee-file (coffee)
  "Return the path to the coffeeScript file named COFFEE."
  (concat "app/coffeescripts/" coffee ".coffee"))

(defun rails-core:sass-file (sass)
  "Return the path to the sass file named SASS."
  (concat "app/stylesheets/" sass ".scss"))

(defun rails-core:css-file (css)
  "Return the path to the css file named CSS."
  (concat "public/stylesheets/" css ".css"))
...
(defun rails-core:current-method-name (&optional point)
  (save-excursion
    (move-end-of-line 1)
    (when (and (search-backward-regexp "^[ ]*def \\([a-z0-9_]+\\)" nil t)
               (or (not point) (< point (point))))

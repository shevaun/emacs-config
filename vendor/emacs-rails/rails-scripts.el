...
(defun rails-script:command (cmd &rest args)
  (apply 'format (concat
...
(defun rails-script:run-color (command parameters &optional buffer-major-mode mode-line-string)
  "Run a Rails script COMMAND with PARAMETERS with
BUFFER-MAJOR-MODE and process-sentinel SENTINEL."
  (rails-script:run-in 'rails-cmd-proxy:start-process-color command parameters buffer-major-mode mode-line-string))
...
(defun rails-script:run-in (process-type command parameters &optional buffer-major-mode mode-line-string)
  ""
  ...
  (rails-project:with-root
   ...
   (save-some-buffers)
   (let ((proc (rails-script:running-p)))
     (if (or proc rails-test:pre-load-test-inprogress)
         (message "Only one instance rails-script allowed")
       (let* ((default-directory root)
              (proc (funcall process-type rails-script:buffer-name
         ...
         (setq rails-script:running-script-name
                   (format "%s %s" command (string-join " " parameters)))
...
(defun rails-script:run-destroy (what &rest parameters)
  ...
  (rails-script:run rails-ruby-command
                    (append (list (rails-script:command "destroy %s"  what))
                            parameters
...
(defun rails-script:run-generate (what &rest parameters)
  ...
  (rails-script:run rails-ruby-command
                    (append (list (rails-script:command "generate %s" what))
                            parameters

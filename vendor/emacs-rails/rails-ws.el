...
(defvar rails-ws:available-servers-list (list "mongrel" "lighttpd" "webrick" "unicorn_rails"))
...
(defun rails-ws:compute-server-conmmand (server-type port env)
  (cond
   ((string= "unicorn_rails" server-type)
    (list server-type
           (format "-p %s -E %s"
                   port
                   env)))
...
(defun rails-ws:restart-passenger ()
  (interactive)
  (rails-test:kill-pre-load-process)
  (let ((filename (rails-core:file "tmp/restart.txt")))
    (if (file-exists-p filename)
        (set-file-times filename)
      (write-region "" nil filename))
    (message "Passenger restart scheduled")))

...
(defun rails-rake:migrate-to-prev-version ()
  ...
  (let ((versions (rails-core:migration-versions t))
        (bn (string-match "^2[0-1][0-9]\\{12\\}" (buffer-name))))
    (if bn
        (let* ((ts (match-string 0 (buffer-name)))
               (func (lambda (list) (if list (if (string= ts (car list)) (rails-rake:migrate (cadr list)) (funcall func (cdr list)))))))
          (funcall func versions))
      (rails-rake:migrate
